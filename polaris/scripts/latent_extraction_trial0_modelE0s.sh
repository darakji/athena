...
CIFS=/eagle/DFTCalculations/mehul/ml/athena/structure_level_latents/fps_seed

OUT_BASE=/eagle/DFTCalculations/mehul/ml/athena/structure_level_latents/fps_seed_cohesive_byMACE
...
for GPU_ID in 0 1 2 3; do
  ...
  CUDA_VISIBLE_DEVICES=${GPU_ID} python ${SCRIPT} \
    --model ${MODEL} \
    --configs ${CIFS} \
    --output ${OUT_BASE}/fps_seed_cohesive_byMACE_${GPU_ID}.extxyz \
    --device cuda \
...
# Merge outputs (safe)
# -------------------------
FINAL_OUT=${OUT_BASE}/fps_seed_cohesive_byMACE.extxyz
rm -f ${FINAL_OUT}

for GPU_ID in 0 1 2 3; do
  PART=${OUT_BASE}/fps_seed_cohesive_byMACE_${GPU_ID}.extxyz
  if [[ -f "${PART}" ]]; then
    cat "${PART}" >> "${FINAL_OUT}"
  else
